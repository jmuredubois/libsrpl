 ...
 */
CamScattering::CamScattering(SRBUF srBuf)
{
...
int CamScattering::CalcSrBuf2Complex(SRBUF scatBuf)
{
	...
	unsigned short* amp = scatBuf.amp;//(WORD*)SR_GetImage(_srCam, 1);	// read current amplitude image
	unsigned short* pha = scatBuf.pha;//(WORD*)SR_GetImage(_srCam, 0);	// read current phase image
...
int CamScattering::CalcComplex2SrBuf(SRBUF scatBuf, NANBUF nanBuf)
{
	...
	unsigned short ampW = 0 ; unsigned short phaW = 0;		// current pixel variables
	unsigned short ampNaN = 0 ; unsigned short phaNaN = 65535;  // values PUT INTO nils
										    // amplitude 0
											// phase 65535 (7.50m)
	for(i=0;i<num;i++)	// for each pixel
    {
		...
		else			// when not NIL
		{
		  ...
		  ampW = (unsigned short) ampdb;		// cast float value into SR WORD
		  phaW = (unsigned short) (65535* atan / (2*pi)); // cast float value into SR WORD
...
int CamScattering::CalcSrBuf2RepComplex(SRBUF scatBuf, int repW, int repH)
{
	...
	unsigned short* amp= scatBuf.amp;// read current amplitude image
	unsigned short* pha= scatBuf.pha;// read current phase image
...
int CamScattering::CalcConv2SrBuf(SRBUF scatBuf, NANBUF nanBuf)
{
	...
	for(i=0;i<num;i++)	// for each pixel
    {
		...
		im = _imgMyKrn[1]->_data[i];	// get imag value
		  ...
		  ampW = (unsigned short) ampf;		// cast float value into SR WORD
		  phaW = (unsigned short) (65535* atan2 / (2*pi)); // cast float value into SR WORD
		  pha[i] = phaW;	// overwrite SR phase with modified value
		  amp[i] = ampW;	// overwrite SR amplitude with modified value
		}
    }
