/*
 *  myConvolveHfloat.h
 * Implementation of the custom convolution method (for large scat kernels)
 *
...
{
	if(!myKern1D ) return;
	if(myKern1D->_coeffs != NULL)
	{
		free(myKern1D->_coeffs);
		myKern1D->_coeffs = NULL;
	}
	return;
}
...
// SECOND convolution : vertical
	...
	if(imgInt != NULL ){ free(imgInt); imgInt=NULL;};
